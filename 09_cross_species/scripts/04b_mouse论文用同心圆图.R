...
# Phase 1 - Script 04b: 论文用同心圆图 (对齐教程美学版)
# -----------------------------------------------------------------------------
# 相对 04 的改动:
#  1. 每个环只画 270 度，留 1/4 空白。
#  2. 环身不再压阶段名；阶段名/n 放在环外留白侧。
#  3. 内嵌折线缩小，放到右上空白扇区。
#  4. 描边更细，中心留白更大，配色加深一档。
#  5. 环按 内->外 = Preneoplastic -> Endpoint；角度方向顺时针。
#
# 科学边界: 弧长不编码细胞总数。每阶段细胞数只作为 n 标注出现。
# =============================================================================
...
pRing <- ggplot(ring_df) +
  ...
  geom_text(
    data = stage_info,
    aes(x = 0.86, y = r_mid, label = label),
    inherit.aes = FALSE,
    hjust = 0,
    size = 2.9,
    fontface = "bold",
  ...
  scale_y_continuous(limits = c(0, r_outer + 0.25), expand = c(0, 0)) +
  guides(fill = guide_legend(override.aes = list(size = 4))) +
  theme_void(base_size = 10) +
  theme(
    legend.position = c(0.04, 0.30),
...
body <- cowplot::ggdraw(pRing) +
  cowplot::draw_plot(inset, x = 0.60, y = 0.62, width = 0.34, height = 0.30)

pR <- cowplot::plot_grid(title_gg, body, ncol = 1, rel_heights = c(0.07, 1))
save_pdf(pR, "R_同心圆亚型组成.pdf", 7.4, 6.6)

writeLines(
  capture.output(sessionInfo()),
  file.path(logs_dir, "04b_论文用同心圆图_sessionInfo.txt")
)
