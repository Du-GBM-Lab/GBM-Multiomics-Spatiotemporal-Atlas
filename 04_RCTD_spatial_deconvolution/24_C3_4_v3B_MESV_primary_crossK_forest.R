#!/usr/bin/env Rscript
# =============================================================================
# R9 | C3-4 v3-B MES-V primary crossK forest
# Scope:
#   Primary response : MES-V
#   Primary statistic: crossK_count delta vs random-labeling null
#   Main cut         : top10
#   Predictors       : vascular vs neuron_control in the same view
#   Scales           : k=6 main, k=12 sensitivity
#
# No new statistics are computed. This script only replots finalized sensitivity
# results after MES-V passed the v3-B A-tier checks.
# =============================================================================
...
fwrite(source_dt, file.path(tab_dir, "R9_C3_4_v3B_MESV_primary_crossK_forest_source.csv"))
...
p <- ggplot() +
  ...
  labs(
    ...
    y = NULL,
    title = "MES-V high spots show local vascular proximity across Visium sections",
    subtitle = "Top 10%; random-labeling null. k=6: vascular median +0.323 (15/18 emp<0.05), neuron -0.187 (1/18). k=12: vascular +0.648 (15/18), neuron -0.494 (2/18).",
    caption = "Despite low spot-level abundance, MES-V independently shows a vascular-proximity signal.\nPositive delta: more vascular-high spots near MES-V-high spots than random expectation. Random-labeling fixed spot positions and point counts; class labels were permuted only."
  ) +
...
pdf_file <- file.path(fig_dir, "R9_C3_4_v3B_crossK_forest_MESV_primary_vascular_vs_neuron.pdf")
png_file <- file.path(fig_dir, "R9_C3_4_v3B_crossK_forest_MESV_primary_vascular_vs_neuron.png")
ggsave(pdf_file, p, width = 9.5, height = 7.2, device = cairo_pdf)
ggsave(png_file, p, width = 9.5, height = 7.2, dpi = 300)

cat("== figure written:\n", pdf_file, "\n", png_file, "\n")
cat("== source data written:\n", file.path(tab_dir, "R9_C3_4_v3B_MESV_primary_crossK_forest_source.csv"), "\n")
