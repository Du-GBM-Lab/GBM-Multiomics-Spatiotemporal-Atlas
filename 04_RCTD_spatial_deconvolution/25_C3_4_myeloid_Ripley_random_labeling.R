#!/usr/bin/env Rscript
# =============================================================================
# R9 | C3-4 phase 1: myeloid proximity re-test with v3-B random-labeling
# Scope:
#   responses  = MES-V (primary), MES-lineage (support), MES-I (control)
#   predictors = myeloid, microglia, macrophage_monocyte, neuron_control
...
#
# Output:
#   Per-slice source data + cross-slice summary + point-count audit.
#   No figures; STOP after numeric report.
# =============================================================================
...
predictors <- c("myeloid", "microglia", "macrophage_monocyte", "neuron_control")
...
message("== C3-4 phase 1 myeloid: random-labeling null; CSR is not used.")
res <- list()
null_audit <- list()
ri <- ai <- 0L
...
fwrite(perslice, file.path(out_dir, "R9_C3_4_myeloid_Ripley_random_labeling_perslice.csv"))
fwrite(summary_all, file.path(out_dir, "R9_C3_4_myeloid_Ripley_random_labeling_summary.csv"))
fwrite(null_audit, file.path(out_dir, "R9_C3_4_myeloid_Ripley_random_labeling_null_audit.csv"))

cat("\n== Myeloid phase summary: MES-V primary, all slices:\n")
print(summary_all[
  sensitivity == "all_slices" &
    response == "MES-V" &
    predictor %in% c("myeloid", "microglia", "macrophage_monocyte", "neuron_control")
...
cat("\n[STOP phase 1 myeloid] Report myeloid/microglia/macrophage-monocyte vs neuron_control,",
    "especially MES-V primary and whether any myeloid compartment passes the vascular-style gate.\n")
