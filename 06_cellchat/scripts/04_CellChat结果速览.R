...
comm_path <- file.path(tables_dir, "CellChat全部通讯结果.csv")
...
write_csv(plaur_ranked, file.path(tables_dir, "CellChat_PLAUR相关通讯_按prob排序.csv"))
...
write_csv(plaur_malignant_tme, file.path(tables_dir, "CellChat_PLAUR恶性亚型_TME通讯.csv"))
...
write_csv(plaur_pair_summary, file.path(tables_dir, "CellChat_PLAUR恶性亚型_TME通讯汇总.csv"))

pathway_rank <- read_csv(file.path(tables_dir, "CellChat_pathway无偏排序.csv"), show_col_types = FALSE) %>%
  mutate(rank = row_number())
pathway_focus <- pathway_rank %>%
  filter(pathway_name %in% c("PLAU", "VTN", "PARs", "THBS", "SPP1", "MIF", "FN1", "COLLAGEN"))
write_csv(pathway_focus, file.path(tables_dir, "CellChat_重点pathway无偏排名.csv"))

writeLines(
  capture.output(sessionInfo()),
  file.path(logs_dir, "04_CellChat结果速览_sessionInfo.txt")
)
